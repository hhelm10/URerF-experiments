# unnormalized, without rotation, default urerf paramters, with noise
...
highdimnoise<-function(high_dim, mag){
...
at_K=seq(10, 15, by=5)
noise_mags = seq(1, 10, by=2)
for (mags in noise_mags){
high_dim_noise_data=highdimnoise(9, mags)
g_noise1=Urerf(high_dim_noise_data, trees = 300, normalizeData=FALSE, Progress = TRUE, splitCrit = "bicfast")
...
final_prec_rf = c(final_prec_rf, D_rf_noise1_precision_list[[1]])
final_rec_rf = c(final_rec_rf, D_rf_noise1_recall_list[[1]])
# generate D_eucd
...
final_prec_iso = c(final_prec_iso, D_iso_noise1_precision_list[[1]])
final_rec_iso = c(final_rec_iso, D_iso_noise1_recall_list[[1]])
...
final_prec_umap = c(final_prec_umap, D_umap_noise1_precision_list[[1]])
final_rec_umap = c(final_rec_umap, D_umap_noise1_recall_list[[1]])
}

print(D_iso_noise1_precision_list)
print(D_umap_noise1_precision_list)
print(D_rf_noise1_precision_list)
save(final_rec_rf, final_prec_rf, final_prec_iso, final_rec_iso, final_prec_umap, final_rec_umap,
     file = "exp7.Rdata")
